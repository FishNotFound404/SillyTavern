...
export async function createWorldInfo(name: string): Promise<unknown> {
  return apiPost('/api/worldinfo/edit', { name, data: { name, entries: {} } })
}
...
export function useSaveWorldInfo() {
  ...
  return useMutation({
    mutationFn: ({ name, data }: { name: string; data: WorldInfoFile }) =>
...
export function useCreateWorldInfo() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: createWorldInfo,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: worldInfoKeys.all })
    },
  })
